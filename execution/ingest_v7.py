...
"""
╔══════════════════════════════════════════════════════════════════════════════╗
║        LEMON SCREENPLAY INGESTION PIPELINE — V7 / FIREBASE EDITION          ║
╚══════════════════════════════════════════════════════════════════════════════╝

Rewrites ingest_screenplays.py for the current production stack:
  • V7 Archaeology Engine  (5-reader parallel → synthesis)
...
─────
  ...
  python execution/ingest_v7.py --source /path/to/pdfs --collection LEMON

  # Single screenplay:
  python execution/ingest_v7.py --source MyScript.pdf --collection SUBMISSION

  # Triage mode (fast, cheap — Haiku single-pass):
  python execution/ingest_v7.py --source /pdfs --collection LEMON --triage

  # Specific model:
  python execution/ingest_v7.py --source /pdfs --collection LEMON --model opus

  # Preview without spending any credits:
  python execution/ingest_v7.py --source /pdfs --collection LEMON --dry-run

  # Skip TMDB check (for scripts you know haven't been produced):
  python execution/ingest_v7.py --source /pdfs --collection LEMON --skip-tmdb

  # Re-analyze scripts already in Firestore:
  python execution/ingest_v7.py --source /pdfs --collection LEMON --force

  # Concurrency (default 3 — safe for API limits):
  python execution/ingest_v7.py --source /pdfs --collection LEMON --concurrency 5
...
LOG_FILE = LOG_DIR / f"ingest_v7_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
# Reader weights (must match src/lib/promptClient.v7.ts READER_WEIGHTS)
READER_WEIGHTS = {
    "structure":          0.40,
    "character":          0.25,
    "craft_scene":        0.15,
...
# ─────────────────────────────────────────────────────────────────────────────
# V8: Rigorous reader prompts + tool schemas + few-shot anchors.
# Methodology ported from agent/skills/screenplay-evaluator/references/
# (the aspirational SKILL.md spec), aligned with src/lib/promptClient.v7.ts
...
Evaluate the structure of the screenplay above using these 12 sub-criteria:
    ...
    "input_schema": {
        ...
        "properties": {
            ...
            "sub_scores": {
                ...
                "required": [
                    "beginning_hook", "middle_build", "ending_payoff",
                    "inciting_incident", "progressive_complications",
...
FAILURE_MODE_ITEM = {
    ...
    "properties": {
        "mode": {"type": "string"},
        "scenes_affected": {"type": "integer", "minimum": 0, "maximum": 5},
    },
        ...
        "properties": {
            ...
            "sub_scores": {
                ...
                "properties": {
                    ...
                    "format_professionalism": SUB_SCORE_SCHEMA,
                },
                "required": [
                    ...
                    "visual_storytelling", "format_professionalism",
                ],
            },
            "bmoc_failure_scan": {
                "type": "object",
...
4. premise_execution_gap — Concept.pillar − avg(Structure, Craft) ≥ 2.0
5. first_act_illusion — Structure.beginning_hook≥7 AND (middle_build<5 OR ending_payoff<5)
6. originality_inflation — Concept.freshness≥7 AND Craft.pillar<5
7. dialogue_disguise — Craft.dialogue_voice_distinction≥7 AND Structure.progressive_complications<5
8. tonal_whiplash — Emotional.emotional_clarity<5 AND Craft.format_professionalism≥6

WARNING (weight 0.0; informational only):
9. second_lead_syndrome — Character.supporting_cast_function≥7 AND star_role_potential<5
...
final_score = (structure × 0.40) + (character × 0.25) + (craft_scene × 0.15)
            + (concept × 0.10) + (emotional_resonance × 0.10)
...
SYNTHESIS_TOOL: Dict[str, Any] = {
    ...
    "input_schema": {
        "type": "object",
        "properties": {
            "analysis_version": {"type": "string", "enum": ["v7_archaeology"]},
            "title": {"type": "string"},
            "author": {"type": "string"},
            "genre": {"type": "string"},
...
def _synthesis_user_blocks(title: str, reader_reports: Dict[str, Any]) -> List[Dict[str, Any]]:
...
# ─── Legacy compatibility shims (used by run_v7_triage and ingest helpers) ──
...
def run_v7_full(
    ...
    """Run the full 5-reader + synthesis V7 pipeline (V8 rigour).
    ...
    syn_user_blocks = _synthesis_user_blocks(title, reader_reports)
    ...
    # Embed reader reports for inspection (preserves the SKILL.md spec output).
    analysis["_reader_reports"] = reader_reports
    analysis["_total_usage"] = total_usage
    analysis["analysis_version"] = analysis.get("analysis_version", "v7_archaeology")
...
def run_v7_hybrid(
    ...
    sonnet_analysis, sonnet_usage = run_v7_full(
        text=text,
        title=title,
    ...
    opus_analysis, opus_usage = run_v7_full(
        text=text,
        title=title,
        page_count=page_count,
        word_count=word_count,
...
def run_v7_triage(
    ...
    triage_prompt = (
        ...
        f"Set should_deep_analyze true if triage_score >= 5.\n"
    ...
    analysis = {
        ...
        "should_deep_analyze": triage.get("should_deep_analyze", score >= 5),
...
def build_raw_document(
    ...
    Mirrors the structure in src/lib/analysisService.ts analyzeV7Path().
    """
    ...
    raw: Dict[str, Any] = {
        ...
        "analysis_version": "v7_archaeology" if mode == "full" else "v7_triage",
...
def ingest_one(
    ...
    try:
        if mode == "triage":
            analysis, usage = run_v7_triage(text, title, page_count, word_count, proxy_url)
        else:
            analysis, usage = run_v7_full(text, title, page_count, word_count, model_key, proxy_url)
    except Exception as e:

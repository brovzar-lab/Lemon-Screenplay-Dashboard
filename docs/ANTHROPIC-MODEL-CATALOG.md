...
Paid `--smoke` is intentionally refused because it does not produce the full trust evidence. The authorized ceiling for this V9 audit is cumulative USD 80, including every settled and uncertain prior call. The one reviewed USD 40 to USD 80 ledger upgrade is atomic, one-way, and accepts only the exact audited snapshot: USD 37.511973 spent across 203 calls, including two uncertain calls totaling USD 7.627776. Every new run must declare the exact settled and uncertain prior spend, use a conservative bounded cap, and run sequentially.

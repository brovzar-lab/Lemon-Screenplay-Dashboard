...
export function FilmDevelopmentPage() {
  const { titles, loading } = useTitleStore()
  const [filterGenre, setFilterGenre] = useState<string>('all')

  const filmTitles = useMemo(
    () => titles.filter(t => t.format === 'feature_film' && t.status !== 'killed'),
    [titles]
  )

  const genres = useMemo(() => {
    const all = filmTitles.flatMap(t => t.genre)
    return Array.from(new Set(all)).sort()
  }, [filmTitles])

  const filtered = useMemo(() => {
    if (filterGenre === 'all') return filmTitles
    return filmTitles.filter(t => t.genre.includes(filterGenre))
  }, [filmTitles, filterGenre])
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-xl font-semibold text-gray-100">Film Development</h1>
          <p className="text-sm text-gray-500 mt-0.5">Feature film projects</p>
        </div>
      ...
      <div className="space-y-6">
        ...
        {!loading && filmTitles.length === 0 && (
          <div className="flex flex-col items-center gap-3 py-16">
            <span className="text-4xl opacity-20">▦</span>
            <p className="text-sm text-gray-500">No feature film titles yet</p>
          </div>
        )}

        {!loading && filmTitles.length > 0 && filtered.length === 0 && (
          <div className="flex flex-col items-center gap-3 py-16">
            <span className="text-4xl opacity-20">◌</span>
            <p className="text-sm text-gray-500">No titles match this genre</p>
            <button
              onClick={() => setFilterGenre('all')}

...
function screenplay(id: string, title: string, weightedScore: number): Screenplay {
  return createTestScreenplay({
    ...
    logline: id === 'delta' ? 'A buried lighthouse signal changes a detective.' : `${title} logline`,
...
describe('I + G screenplay Discovery presentation', () => {
  ...
  it('keeps a cinematic feature and the complete filtered slate in the grid', async () => {
    ...
    expect(await screen.findByTestId('screenplay-discovery-featured')).toHaveAttribute('data-screenplay-id', 'echo');
  ...
  it('uses the existing search and sort machinery across feature and grid', async () => {
    ...
    await waitFor(() => expect(screen.getAllByTestId('screenplay-discovery-result')).toHaveLength(1));
    expect(within(screen.getByTestId('screenplay-discovery-featured')).getByRole('heading')).toHaveTextContent('Delta Run');

    await user.clear(search);
    await user.selectOptions(screen.getByRole('combobox', { name: 'Sort results' }), 'title');
    await waitFor(() => expect(within(screen.getByTestId('screenplay-discovery-featured')).getByRole('heading')).toHaveTextContent('Amber Sky'));
  });
  ...
  it('opens the connected Screenplay File and preserves the drawer fallback', async () => {
    ...
    expect(screen.getByTestId('location-probe')).toHaveTextContent('/projects/echo-project?workspace=screenplay');
    connected.unmount();

    renderPage('/discover?ui=screenplay&preview=drawer');
    await user.click(await screen.findByRole('button', { name: 'Open Echo Park screenplay file' }));
    expect(screen.getByTestId('location-probe')).toHaveTextContent('/discover/echo-project?ui=screenplay&preview=drawer');
  });
});

  ...
  return (
    <main className="min-h-screen flex items-center justify-center" style={{ background: 'var(--sp-bg)' }}>
      <div className="text-center" role="status">
        ...
        <p className="text-sm" style={{ color: 'var(--sp-text-3)' }}>{t('Opening Lemon Studios...')}</p>
      </div>
    </main>
  );
}
    ...
    return (
      <main className="min-h-screen flex items-center justify-center px-6" style={{ background: 'var(--sp-bg)' }}>
        <section className="text-center max-w-md">
          <h1 className="text-2xl font-display mb-3" style={{ color: 'var(--sp-text)' }}>{t('Admin access required')}</h1>

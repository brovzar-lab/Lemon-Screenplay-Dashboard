...
export function ExportModal({
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      if (isDiscovery) {
        ...
        // Preserve the dashboard's existing auto-close behavior.
        setTimeout(() => {
          setIsExporting(false);
          setExportProgress(0);
          onClose();
  ...
  return (
    ...
    >
      ...
      <div
        className={
          isDiscovery ? 'fixed inset-0 dsc-scrim' : 'fixed inset-0 bg-black-950/80 backdrop-blur-sm'
        }
        onClick={onClose}
      />
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-labelledby="export-screenplays-title"
      ...
      >
          ...
          <button
            ...
            className={
              isDiscovery
                ...
                : 'p-1 rounded hover:bg-black-700 text-black-400 hover:text-gold-400'
            }
          >
        ...
        <div className={clsx('space-y-4', isDiscovery ? 'p-5 sm:p-6' : 'p-4')}>
          ...
          <div
            className={clsx(
              'p-3',
              isDiscovery ? 'dsc-row rounded-xl' : 'rounded-lg bg-black-900/50',
            )}
          >
              ...
              <ul className="mt-2 space-y-1">
                {screenplays.map((sp) => (
                  <li
                    key={sp.id}
                    className={clsx(
                      'text-xs text-black-300 flex items-center gap-2',
                      isDiscovery && 'text-[var(--dsc-ink-2)]',
                    )}
                  >
                    <span
                      className={clsx(
                        ...
                        sp.recommendation === 'recommend' &&
                          (isDiscovery ? 'bg-[var(--dsc-success)]' : 'bg-emerald-500'),
                        sp.recommendation === 'consider' &&
                          (isDiscovery ? 'bg-[var(--dsc-consider)]' : 'bg-amber-500'),
                        sp.recommendation === 'pass' &&
                          (isDiscovery ? 'bg-[var(--dsc-pass)]' : 'bg-red-500'),
                      )}
                    />
                    {sp.title}
                  </li>
          ...
          {!pdfOnly && (
            <div>
              <label
                className={clsx(
                  'text-sm font-medium text-black-400 block mb-2',
              ...
              <div className="grid grid-cols-2 gap-3">
                <button
                  ...
                  className={clsx(
                    ...
                    format === 'pdf'
                      ...
                      : isDiscovery
                        ? 'border-[var(--dsc-line)] hover:border-[var(--dsc-accent)]'
                ...
                >
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <span
                      className={clsx(
                        'text-sm font-medium text-black-200',
                  ...
                  <div className="flex flex-col items-center gap-2">
                    <svg
                      className={clsx(
                        ...
                        isDiscovery && 'text-[var(--dsc-success)]',
                      )}
                      fill="currentColor"
                      viewBox="0 0 24 24"
                    >
          ...
          <div
            className={clsx(
              ...
              isDiscovery
                ? 'dsc-row rounded-xl'
                : 'rounded-lg border border-black-700 bg-black-900/30',
            )}
          >
            {effectiveFormat === 'pdf' ? (
              <div className={clsx('text-xs text-black-400', isDiscovery && 'dsc-muted')}>
                <p
                  className={clsx(
                    'font-medium text-black-300 mb-1',
                    isDiscovery && 'text-[var(--dsc-ink)]',
                  )}
                >
          ...
          {isExporting && (
            <div className="space-y-2">
              ...
              <div
                className={
                  isDiscovery
                    ? 'h-2 overflow-hidden rounded-full bg-[var(--dsc-surface-2)]'
                    : 'h-2 bg-black-800 rounded-full overflow-hidden'
                }
              >
                <div
                  className={
                    isDiscovery
                      ? 'h-full bg-[var(--dsc-accent)] transition-all duration-300'
          ...
          {showInlineFailure && exportError && (
            <p
              role="alert"
              className={clsx(
                'border border-red-500/30 bg-red-500/10 px-3 py-2 text-sm text-red-300',
                isDiscovery &&
                  'border-[var(--error)] bg-[var(--error-soft)] text-[var(--on-error)]',
              )}
            >
              {exportError}
            </p>
        ...
        <div
          className={clsx(
            'flex items-center justify-end gap-3 border-t border-black-700 p-4',
            isDiscovery && 'dsc-modal-footer',
            !isDiscovery && 'bg-black-900/30',
          )}
        >
          {isDiscovery && exportComplete ? (

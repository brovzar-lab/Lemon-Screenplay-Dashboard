  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-black-800 border border-gold-500/10 rounded-xl p-6">
        ...
        <ScoreBar
          label={projection ? t('Final score') : t('Legacy stored score')}
          score={weightedScore}
        />
        {projection && (
          <div className="mt-4 grid gap-2 border-t border-black-700 pt-4 text-sm">
            <div className="flex justify-between gap-4 text-black-300">
              <span>
                {projection.scoreSource === 'triage'
                  ? t('Raw triage score')
                  : t('Raw five-pillar score')}
              </span>
              <strong>{projection.rawScore.toFixed(2)}</strong>
            </div>
            <div className="flex justify-between gap-4 text-black-300">
              <span>{t('Critical-failure deduction applied')}</span>
              <strong>
            ...
            {Math.abs(
              ...
              <div className="flex justify-between gap-4 text-black-300">
                <span>{t('Critical-failure issues reported')}</span>
        ...
        {!projection && (
          <p className="mt-4 border-t border-black-700 pt-4 text-sm leading-6 text-black-400">
            {t('This older shared snapshot does not preserve adjusted-score lineage, so the stored score cannot be verified as a final adjusted score.')}
          </p>
        )}
      </div>
      ...
      <div className="bg-black-800 border border-gold-500/10 rounded-xl p-6">
        <h3 className="text-sm font-semibold text-gold-200 uppercase tracking-wider mb-4">
          {pillars.length > 0 ? t('Five-Pillar Reader Evidence') : t('Legacy Dimension Scores')}
        </h3>
        <div className="space-y-4">
          {pillars.length > 0
            ? pillars.map((pillar) => (
                <ScoreBar
                  key={pillar.name}
                  label={`${t(pillarLabels[pillar.name] ?? pillar.name)} (${(pillar.weight * 100).toFixed(0)}%)`}
                  score={pillar.score}
                />
              ))
            ...
            return (
              <ScoreBar
                key={key}
                label={`${t(label)} (${(weight * 100).toFixed(0)}%)`}
                score={score}

...
const BulkReanalyzeModal = lazy(() =>
  import('@/components/bulk/BulkReanalyzeModal').then((module) => ({
    default: module.BulkReanalyzeModal,
  })),
);
...
export function FilterBar({
  ...
  const advancedFilterCount = useFilterStore(
    (s) =>
      [
    ...
    <>
      <div className="mb-8">
        <div className="flex flex-col gap-3">
            ...
            <div className="relative w-full sm:w-auto">
              ...
              {!searchQuery && (
                <span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-black-600 pointer-events-none hidden md:inline-flex items-center gap-1">
                  <kbd className="px-1.5 py-0.5 rounded bg-black-700 border border-black-600 font-mono">
                    /
                  </kbd>
                </span>
              )}
              <span className="absolute left-3 top-1/2 -translate-y-1/2 text-black-500">🔍</span>
              {searchQuery && (
                <button
          ...
          {/* Action buttons row — scrollable on mobile, wrapped on wider screens */}
          <div className="flex items-center gap-2 overflow-x-auto scrollbar-hide w-full pb-1 lg:flex-wrap lg:overflow-visible lg:pb-0">
            {/* Quick Sort Dropdown */}
            <select
            ...
            <button
              onClick={() =>
                isAllSelected
                  ? deselectAllExport()
                  : selectAllForExport(screenplays.map((sp) => sp.id))
              }
              className="btn btn-secondary text-sm shrink-0 min-h-[44px]"
        ...
        <nav aria-label="Filter screenplays" className="overflow-x-auto scrollbar-hide mt-4">
          <div
            ...
            className="relative flex flex-nowrap sm:flex-wrap gap-2 pb-1 min-w-max sm:min-w-0"
          >
            ...
            {FILTER_CHIPS.map((chip) => (
              <button
                ...
                className={`chip cursor-pointer transition-all ${
                  activeFilter === chip.id ? chip.activeClass : ''
                }`}
                style={
            ...
            {(missingPdfCount > 0 || missingPdfOnly) && (
              <button
                ...
                className={clsx(
                  ...
                  missingPdfOnly ? 'font-semibold' : '',
                )}
                style={
                  missingPdfOnly
                    ? {
                        background: 'var(--sp-consider-tint)',
                        color: 'var(--sp-consider)',
                        fontWeight: 600,
                      }
                    : undefined
                }
              >
                ...
                {missingPdfCount > 0 && (
                  <span
                    ...
                    style={{ background: 'var(--sp-consider-tint)', color: 'var(--sp-consider)' }}
                  >
                    {missingPdfCount}
                  </span>
                )}
            ...
            <button
              ...
              className={clsx(
                'chip cursor-pointer transition-all',
                hasPdfOnly ? 'font-semibold' : '',
              )}
              style={
                hasPdfOnly
                  ? {
                      background: 'var(--sp-recommend-tint)',
                      color: 'var(--sp-recommend)',
                      fontWeight: 600,
                    }
                  : undefined
              }
            >
              ...
              {hasPdfCount > 0 && (
                <span
                  className="ml-1 px-1.5 py-0.5 rounded-full text-xs font-bold"
                  style={{ background: 'var(--sp-recommend-tint)', color: 'var(--sp-recommend)' }}
                >
                  {hasPdfCount}
                </span>
              )}
      ...
      <AdvancedSortPanel isOpen={isSortPanelOpen} onClose={() => setIsSortPanelOpen(false)} />
      ...
      {isAdmin && <BadFormatModal open={isBadFormatOpen} onClose={() => setBadFormatOpen(false)} />}
    </>
  );
}

      ...
      <div className="mb-3 flex flex-wrap items-end justify-between gap-2">
        <div>
          ...
          <p className={clsx('mt-1 text-xs', isIntake ? 'text-[var(--dsc-ink-3)]' : 'text-black-400')}>
            {t('Approved routes verified {{date}}. New releases require benchmark approval before scoring changes.', { date: new Date(`${modelCatalog.verifiedAt}T12:00:00`).toLocaleDateString(i18n.language === 'es' ? 'es-MX' : 'en-US', { month: 'short', day: 'numeric', year: 'numeric' }) })}
          </p>
        </div>
          ...
          >
            ...
            <div className="flex items-center gap-2 mb-2">
              ...
              <div>
                ...
                <p className={clsx('text-xs', isIntake ? 'text-[var(--dsc-ink-3)]' : 'text-black-400')}>{t(model.subtitle)}</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-3 gap-2 mt-3 mb-3">
              <div className="text-center">
                <p className={clsx('mb-0.5 text-xs', isIntake ? 'text-[var(--dsc-ink-3)]' : 'text-black-500')}>{t('Cost')}</p>
                <p className={clsx(
                  'text-sm font-bold',
              ...
              <div className="text-center">
                <p className={clsx('mb-0.5 text-xs', isIntake ? 'text-[var(--dsc-ink-3)]' : 'text-black-500')}>{t('Speed')}</p>
                <p className={clsx('text-sm', isIntake ? 'text-[var(--dsc-ink-2)]' : 'text-black-300')}>{model.speed}</p>
              </div>
              <div className="text-center">
                <p className={clsx('mb-0.5 text-xs', isIntake ? 'text-[var(--dsc-ink-3)]' : 'text-black-500')}>{t('Quality')}</p>
                <p className={clsx('text-sm', isIntake ? 'text-[var(--dsc-ink-2)]' : 'text-black-300')}>{t(model.quality)}</p>
              </div>
            </div>
            ...
            <p className={clsx('mt-3 border-t pt-2 font-mono text-[10px] leading-relaxed', isIntake ? 'border-[var(--dsc-line)] text-[var(--dsc-ink-3)]' : 'border-black-700 text-black-500')}>
              <span className="font-sans font-bold uppercase tracking-[0.08em]">{t(model.routeLabel)}</span>
              <br />
              {model.modelId}
            </p>
      ...
      {pendingCount > 0 && batchCostEstimate && (
        <div className="mt-3 flex items-center gap-2 text-sm">
          ...
          <span className={isIntake ? 'text-[var(--dsc-ink-2)]' : 'text-black-400'}>
            {t('Estimated batch cost for {{count}} files with {{model}}:', { count: pendingCount, model: MODEL_OPTIONS.find(m => m.id === selectedModel)!.name })}{' '}

...
export function FavoritesPanel() {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h2 className="text-xl font-display text-gold-200 mb-2">{t('Favorites')}</h2>
        <p className="text-sm text-black-400">
          {t('Organize your favorite screenplays into custom lists for easy access.')}
        </p>
      </div>
      ...
      <div className="p-4 rounded-xl bg-black-800/50 border border-black-700">
        <div className="flex items-center gap-3 mb-3">
          ...
          <h3 className="text-lg font-medium text-gold-200">{t('Quick Favorites')}</h3>
        </div>
        <p className="text-sm text-black-400 mb-2">
          {t('{{count}} screenplay saved as a quick favorite', { count: quickFavorites.length })}
        </p>
          ...
          <div className="mt-3 space-y-1">
            {quickFavorites.slice(0, 10).map(id => (
              <p key={id} className="text-xs text-gold-200/70 truncate pl-2 border-l-2 border-gold-500/20">
                {titleMap.get(id) || <span className="text-black-500 italic">{t('Unknown screenplay')}</span>}
              </p>
            ))}
            {quickFavorites.length > 10 && (
              <p className="text-xs text-black-500 pl-2">{t('+{{count}} more', { count: quickFavorites.length - 10 })}</p>
            )}
          </div>
        )}
        <p className="text-xs text-black-500 mt-2">
          {t('Click the star icon on any screenplay card to add it here.')}
        </p>
      </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-medium text-gold-200">{t('Custom Lists')}</h3>
          <button
        ...
        {lists.length === 0 ? (
          <div className="text-center py-8 text-black-500">
            ...
            <p>{t('No custom lists yet')}</p>
            <p className="text-sm mt-1">{t('Create a list to organize your favorite screenplays')}</p>
          </div>
        ) : (
                    ...
                    {editingId === list.id ? (
                      ...
                      <>
                        ...
                        <p className="text-xs text-black-500">
                          {t('{{count}} screenplay', { count: list.screenplayIds.length })} · {t('Updated')} {new Date(list.updatedAt).toLocaleDateString(i18n.language === 'es' ? 'es-MX' : 'en-US')}
                        </p>
                      </>
                    )}
                  </div>
                  ...
                  <div className="ml-14 mt-1 mb-2 pl-3 border-l-2 border-gold-500/15 space-y-1">
                    {list.screenplayIds.map(id => (
                      <p key={id} className="text-xs text-gold-200/60 truncate">
                        {titleMap.get(id) || <span className="text-black-500 italic">{t('Unknown')}</span>}
                      </p>
                    ))}
                  </div>
                )}
                {expandedId === list.id && list.screenplayIds.length === 0 && (
                  <p className="ml-14 mt-1 mb-2 text-xs text-black-500 italic">{t('Empty list')}</p>
                )}
              </div>
            ))}
          </div>
      ...
      <div className="p-4 rounded-lg bg-black-800/30 border border-black-700">
        <h4 className="text-sm font-medium text-gold-300 mb-2">{t('How to use lists')}</h4>
        <ul className="text-sm text-black-400 space-y-1">
          <li>• {t('Click the bookmark icon on any screenplay to add it to a list')}</li>
          <li>• {t('Use the filter panel to show only screenplays from a specific list')}</li>
          <li>• {t('Export lists as CSV or JSON for external use')}</li>
        </ul>
      </div>
      ...
      <ConfirmDialog
        ...
        message={deleteTarget && deleteTarget.screenplayIds.length > 0
          ? t('This will permanently remove this list and unlink {{count}} screenplays from it. The screenplays themselves will not be deleted.', { count: deleteTarget.screenplayIds.length })
          : t('This will permanently remove this list. The screenplays themselves will not be deleted.')}

...
export function AnalysisOverview() {
  ...
  const metrics = [
    {
      label: 'Complete reader panels',
      value: health.completePanels,
      detail: `${percentage(health.completePanels, health.total)} of the slate has every expected reader`,
    },
    {
      label: 'Verified evidence',
      value: health.verified,
      detail: `${percentage(health.verified, health.total)} has current evidence lineage`,
    },
    {
      label: 'Needs review',
    ...
    },
    {
      ...
      detail: `${percentage(health.pdfReady, health.total)} can open the original screenplay`,
    },
  ];
  ...
  return (
    <div className="space-y-7">
      <header>
        ...
        <h2 className="mt-2 text-3xl font-display text-black-100">Know what can be trusted</h2>
        <p className="mt-2 max-w-3xl text-sm leading-6 text-black-400">
          This view reports the health of the analyses already in the slate. It does not run a new
          analysis or alter a score.
        </p>
      </header>
        ...
        {metrics.map((metric) => (
          ...
          >
            ...
            <strong className="mt-2 block text-3xl tabular-nums text-black-100">
              {metric.value}
              <span className="ml-1 text-sm font-normal text-black-500">of {health.total}</span>
            </strong>
      ...
      <section className="grid gap-5 rounded-xl border border-black-700 bg-black-900/30 p-5 lg:grid-cols-[0.7fr_1.3fr] lg:p-6">
        <div>
          ...
          <div className="mt-4 grid grid-cols-2 gap-3">
            ...
            <div className="rounded-lg border border-black-700 bg-black-950/20 p-4">
              <span className="text-xs text-black-500">Most common genre</span>
              <strong className="mt-1 block text-base leading-5 text-black-100">
                {health.topGenre[0]}
              </strong>
              <small className="text-black-500">{health.topGenre[1]} projects</small>
            </div>
          </div>
        </div>
      ...
      <section className="rounded-xl border border-black-700 bg-black-900/30 p-5 sm:p-6">
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            <h3 className="text-xl font-display text-black-100">Attention required</h3>

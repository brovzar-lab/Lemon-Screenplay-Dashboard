...
export function PdfUploadPanel() {
  ...
  const {
    setBulkStatuses,
    setStatus: setPdfStoreStatus,
    setIsScanning: setPdfStoreScanning,
  } = usePdfStatusStore();
  ...
  const [uploadEntries, setUploadEntries] = useState<
    Record<string, { state: UploadState; error?: string }>
  >({});
  ...
  const checkStoragePaths = useCallback(
    async (list: Screenplay[]) => {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="relative flex-1">
          <svg
            className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-black-500"
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
          >
        ...
        {filteredScreenplays.map((screenplay) => {
          ...
          return (
            <div
              key={screenplay.id}
              className={clsx(
                'settings-file-row flex items-center gap-3 p-3 rounded-xl border transition-all',
                isSelected
            ...
            >
              ...
              {/* Category badge */}
              <span className="settings-file-category shrink-0 text-xs px-2 py-0.5 rounded-full">

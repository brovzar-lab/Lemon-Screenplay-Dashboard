...
export function ReanalyzeButton({ screenplay, onComplete }: ReanalyzeButtonProps) {
  ...
  const handleReanalyze = async (modelId: 'sonnet' | 'opus') => {
    ...
    try {
      ...
      await reanalyzeFromStorage(screenplay, modelId, (p) => setProgress(p));
      ...
      setError(t('Re-analysis failed. Please try again.'));
      console.error('[ReanalyzeButton]', err);
      useToastStore.getState().addToast(t('Reanalysis failed — please try again'));
    } finally {
    ...
    return (
      <div className="flex items-center gap-2 text-xs text-gold-400">
        ...
        <span className="max-w-[180px] truncate">
          {progress?.message ? t(progress.message) : t('Starting re-analysis...')}
        </span>
      </div>
    );
  }
      ...
      {isOpen && (
        <div className="absolute top-full right-0 mt-1 w-56 py-1 bg-black-800 border border-black-600 rounded-lg shadow-xl z-50">
          ...
          <div className="px-3 py-1.5 text-[10px] text-black-500 border-b border-black-700">
            {t('Current')}: <span className="text-black-300">{t(currentVersionLabel)}</span>
          </div>
          ...
          {REANALYSIS_MODELS.map((model) => {
            return (
              ...
              >
                <span className="text-sm text-black-200 group-hover:text-gold-300">
                  {model.label}
                </span>
                <span className="text-[10px] text-black-500">{t(model.desc)}</span>
              </button>
            );
          })}
        </div>

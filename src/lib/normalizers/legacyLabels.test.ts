...
describe('legacy analysis_version acceptance', () => {
  ...
  it('shows only a validated poster from the latest screenplay version', () => {
    const doc = archaeologyDoc('v9_archaeology');
    doc.latest_version_id = 'v2';
    doc.poster_version_id = 'v2';
    doc.poster_url = 'https://example.com/current.png';
    doc.poster_status = 'ready';

    expect(normalizeV9Screenplay(doc, 'LEMON').posterUrl).toBe(doc.poster_url);

    doc.poster_version_id = 'v1';
    expect(normalizeV9Screenplay(doc, 'LEMON').posterUrl).toBeUndefined();

    doc.poster_status = 'invented';
    expect(normalizeV9Screenplay(doc, 'LEMON').posterStatus).toBeUndefined();
  });
});

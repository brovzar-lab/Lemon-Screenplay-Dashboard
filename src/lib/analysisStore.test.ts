...
function resetTransactionMock(): void {
  ...
  mockRunTransaction.mockImplementation(async (_db: unknown, update: (tx: unknown) => unknown) => {
...
describe('analysisStore authReady gates', () => {
  ...
  it('atomically creates a typed immutable version and advances its parent', async () => {
    ...
    expect((versionDocument.analysis as Record<string, unknown>).reader_reports).toBeDefined();
    expect((parentDocument.analysis as Record<string, unknown>).reader_reports).toBeUndefined();
...
describe('flushPendingWrites export', () => {
  ...
  it('awaits authReady before retrying queued Firestore writes', async () => {
    ...
    expect(callOrder.indexOf('setDoc')).toBeGreaterThan(callOrder.indexOf('authReady:resolved'));
...
describe('patchAnalysisField', () => {
  ...
  it('keeps the local edit and queues it when Firestore is unavailable', async () => {
    ...
    const { default: currentI18n } = await import('@/i18n');
    const { useToastStore: currentToastStore } = await import('@/stores/toastStore');
    await currentI18n.changeLanguage('es');
    currentToastStore.getState().clearToasts();
    ...
    expect(currentToastStore.getState().toasts.at(-1)?.message).toBe(
...
describe('quarantineAnalysis', () => {
  ...
  it('marks the source document as quarantined without deleting it', async () => {
    localStore['lemon-local-analyses'] = JSON.stringify([{ source_file: 'bad.pdf', title: 'Bad' }]);

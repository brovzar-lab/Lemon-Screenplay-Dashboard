...
exports.PUBLIC_SHARE_MANIFEST_VERSION = exports.SHARE_AUTHORITY_VERSION = void 0;
exports.buildSharedViewRecord = buildSharedViewRecord;
exports.buildShareAuthorityRecord = buildShareAuthorityRecord;
exports.resolveAuthoritativeShare = resolveAuthoritativeShare;
const node_crypto_1 = require("node:crypto");
const calibrationCore_1 = require("./calibrationCore");
exports.SHARE_AUTHORITY_VERSION = "lemon-share-authority-v1";
exports.PUBLIC_SHARE_MANIFEST_VERSION = "lemon-public-share-manifest-v1";
...
function textList(value, limit = 100) {
...
function publicAnalysisProjection(value) {
    ...
    const comparableFilms = Object.fromEntries(["tone", "structure", "market"].flatMap((name) => {
    ...
    return {
        ...
        ...(finiteNumber(analysis.critical_failure_penalty_applied) !== undefined
            ? {
                critical_failure_penalty_applied: finiteNumber(analysis.critical_failure_penalty_applied),
            }
            : {}),
        ...(finiteNumber(analysis.critical_failure_total_penalty) !== undefined
            ? {
                critical_failure_total_penalty: finiteNumber(analysis.critical_failure_total_penalty),
            }
            : {}),
...
function publicLocalizedAnalysis(value, versionId) {
    ...
    if (sourceVersionId !== versionId || !generatedAt || !model)
        return undefined;
    ...
    const publicContent = {
        ...
        dimensionJustifications: Object.fromEntries(["concept", "structure", "protagonist", "supportingCast", "dialogue", "genreExecution", "originality"]
            .flatMap((key) => {
            const valueText = text(dimensionJustifications[key], 10_000);
            return valueText ? [[key, valueText]] : [];
        })),
        commercialViabilityNotes: Object.fromEntries(["targetAudience", "highConcept", "castAttachability", "marketingHook", "budgetReturnRatio", "comparableSuccess"]
            .flatMap((key) => {
...
function buildPublicTrustProjection(projectId, versionId, version, publicAnalysis, localizedAnalysis) {
    ...
    const unsigned = {
        manifest_version: exports.PUBLIC_SHARE_MANIFEST_VERSION,
...
function publicMetadata(version) {
...
function buildSharedViewRecord(input) {
    const parent = asRecord(input.parent);
    const version = asRecord(input.version);
    (0, calibrationCore_1.extractProducerAnalysisSnapshot)(input.projectId, input.versionId, version, input.versionAuthority);
    ...
    const localizedAnalysis = publicLocalizedAnalysis(version.localized_analysis, input.versionId);
    const publicTrust = buildPublicTrustProjection(input.projectId, input.versionId, version, publicAnalysis, localizedAnalysis);
    ...
    const notes = input.includeNotes && Array.isArray(input.notes)
        ...
        : [];
    return {
        authorityVersion: exports.SHARE_AUTHORITY_VERSION,
        token: input.token ?? (0, node_crypto_1.randomUUID)(),
        screenplayId: text(input.screenplayId, 500),
        screenplayTitle: text(asRecord(version.analysis).title ?? version.source_file, 500),
        ...
        sealedVersion: {
            ...
            trust_manifest_version: exports.PUBLIC_SHARE_MANIFEST_VERSION,
            analysis: publicAnalysis,
...
function buildShareAuthorityRecord(input) {
    const version = asRecord(input.version);
...
function resolveAuthoritativeShare(input) {
    ...
    if (share.authorityVersion !== exports.SHARE_AUTHORITY_VERSION
        || authority.authorityVersion !== exports.SHARE_AUTHORITY_VERSION
...
//# sourceMappingURL=shareCore.js.map
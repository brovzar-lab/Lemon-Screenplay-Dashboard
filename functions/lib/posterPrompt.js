...
exports.buildSimplePosterPrompt = buildSimplePosterPrompt;
const GENRE_VISUAL_DNA = {
    horror: {
...
// ---------------------------------------------------------------------------
function selectArchetype(genre, preferredArchetype) {
...
// ---------------------------------------------------------------------------
function resolveGenreDNA(genre) {
    const genreLower = genre.toLowerCase();
    // Direct match
    if (GENRE_VISUAL_DNA[genreLower])
        return GENRE_VISUAL_DNA[genreLower];
    ...
    for (const word of words) {
        if (GENRE_VISUAL_DNA[word])
            return GENRE_VISUAL_DNA[word];
    }
...
// ---------------------------------------------------------------------------
function extractVisualHooks(logline) {
...
function getRegionalNotes(options) {
    if (!options)
        return '';
    const notes = [];
    ...
    if (options.marketFeel === 'latin_american') {
        notes.push('Visual sensibility should reflect Latin American cinema — grounded realism with magical undercurrents, ' +
    ...
    if (options.marketFeel === 'arthouse') {
        notes.push('Art-house aesthetic: restrained, conceptual, European poster design sensibility. ' +
...
// ---------------------------------------------------------------------------
function getTaglineInstruction(tagline) {
    if (!tagline)
        return '';
...
function buildPosterPrompt(options) {
    const { title, logline, genre, tagline, archetype: preferredArchetype, regional, additionalNotes, tone, } = options;
...
// ===========================================================================
function buildSimplePosterPrompt(title, logline, genre) {
...
*/
//# sourceMappingURL=posterPrompt.js.map
...
const GENRE_TOOL_DESCRIPTION = ("Submit the screenplay's validated Story Grid genre classification.");
const GENRE_SCHEMA_SHA256 = ("bf44522ad93f51c292ee622422752a5e1fb0ad54276c17ba9e69d8e837ff7645");
...
function isExactToolChoice(value, expectedToolName, allowAuto) {
    if (!isRecord(value))
        return false;
    if (allowAuto) {
...
function isExactCompactPayload(payload, expectedToolName, allowAutoToolChoice) {
    const tools = payload.tools;
    ...
    return Boolean(tool
        ...
        && tool.strict === true
        && isExactToolChoice(payload.tool_choice, expectedToolName, allowAutoToolChoice)
        && schema
...
function isExactGenrePayload(payload) {
    const tools = payload.tools;
    const tool = Array.isArray(tools) && tools.length === 1 && isRecord(tools[0])
        ? tools[0] : null;
    const schema = tool && isRecord(tool.input_schema) ? tool.input_schema : null;
    return Boolean(tool
        ...
        && tool.strict === true
        && schema
        && (0, anthropicProxyCore_1.sha256CanonicalJson)(schema) === GENRE_SCHEMA_SHA256
...
function isPrimitiveEnum(values) {
...
function isStrictCorrectionSchema(value, depth = 0) {
    if (!isRecord(value) || depth > 16 || typeof value.type !== "string")
        return false;
    ...
    if (type === "object") {
        ...
        if (propertyNames.some((name) => !SAFE_SCHEMA_FIELD.test(name)))
            return false;
        const required = value.required;
        if (JSON.stringify([...required].sort()) !== JSON.stringify(propertyNames.sort())) {
            return false;
        }
        return propertyNames.every((name) => isStrictCorrectionSchema(properties[name], depth + 1));
    }
    if (type === "array") {
    ...
    if (value.enum !== undefined)
        allowedKeys.push("enum");
    if (!hasExactKeys(value, allowedKeys))
        return false;
    return value.enum === undefined || isPrimitiveEnum(value.enum);
}
function isTargetedCorrectionPayload(payload, expectedToolName, allowAutoToolChoice) {
    const tools = payload.tools;
    const tool = Array.isArray(tools) && tools.length === 1 && isRecord(tools[0])
        ? tools[0] : null;
    const schema = tool && isRecord(tool.input_schema) ? tool.input_schema : null;
...
function validateBenchmarkContract(value, evidence, expectedRunId, requestModel, payload) {

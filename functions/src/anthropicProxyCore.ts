...
import {
  ...
  type ApprovedEffort,
  type ProxyCallerKind,
} from "./llmProxyPolicy";
import type { LlmTokenUsage } from "./llmCost";
...
export class ProxyRequestValidationError extends Error {
  constructor(
    message: string,
    readonly code: "INVALID_INPUT" | "INVALID_MODEL" = "INVALID_INPUT",
  ) {
    super(message);
...
function extractSystem(body: ProxyRequestBody): ProxyRequestBody["system"] {
  ...
  const hasBlocks = systemMessages.some(
    (message) => Array.isArray(message.content)
      && message.content.some(
        (block) => block.type === "text" && block.cache_control !== undefined,
      ),
  );
  if (!hasBlocks) {
...
export function buildAnthropicRequest(
  ...
  const body = value as ProxyRequestBody;
  if (typeof body.model !== "string" || !Array.isArray(body.messages)) {
    throw new ProxyRequestValidationError("Missing required fields: model, messages");
  }
  let modelOutputLimit: number;
  try {
    modelOutputLimit = approvedMaxOutputTokens(body.model, operationalOutputCap);
  } catch {
  ...
  if (!Number.isInteger(maxTokens) || maxTokens < 1 || maxTokens > modelOutputLimit) {
    throw new ProxyRequestValidationError(
      `max_tokens must be an integer between 1 and ${modelOutputLimit}.`,
    );
  }
  if (body.thinking?.type === "enabled"
    ...
    throw new ProxyRequestValidationError(
      `thinking.budget_tokens must be between 1 and ${thinkingTokenCap}.`,
    );
  }
  let outputConfig: { effort: ApprovedEffort } | undefined;
  try {
    outputConfig = validateModelRequest(body.model, caller, {
    ...
    throw new ProxyRequestValidationError(
      message,
      message === "Model is not approved." ? "INVALID_MODEL" : "INVALID_INPUT",
    );
  }

...
export type ReaderPosition = "unchanged" | "clarified" | "reconsidered";
...
const READER_CONFIG: Record<ReaderKey, {
  reportKey: string;
  charterFile: string;
  displayName: string;
  role: string;
...
function requiredString(value: unknown, field: string, max = 20_000): string {
...
export function parseReaderKey(value: unknown): ReaderKey {
  if (
    value === "structure"
    || value === "character"
    || value === "craft"
...
export function readerIdentity(reader: ReaderKey) {
  return { ...READER_CONFIG[reader] };
}

export function loadReaderCharter(reader: ReaderKey): {
  text: string;
  version: string;
  sha256: string;
} {
  const path = resolve(
    ...
    "reader-charters",
    "v1",
    READER_CONFIG[reader].charterFile,
  );
  const text = readFileSync(path, "utf8").trim();
  return {
    text,
    version: READER_CHARTER_VERSION,
...
export function readerReportFromVersion(
  version: unknown,
  reader: ReaderKey,
): UnknownRecord {
...
export function screenplayStoragePointer(version: unknown): {
  bucket?: string;
  objectName: string;
  generation?: string;
} {
...
export function buildReaderSystemPrompt(input: {
  reader: ReaderKey;
  charter: string;
  charterSha256: string;
  sealedReport: UnknownRecord;
  sharedSynthesis: unknown;
  projectId: string;
  versionId: string;
  title: string;
}): string {
...
export function buildConversationHistory(messages: ConversationMessage[]): string {
  if (!messages.length) return "No prior private conversation.";
...
export const READER_REPLY_TOOL = {
...
export function parseReaderReply(value: unknown, pageCount?: number): ReaderReply {
  ...
  const citations = Array.isArray(record.citations)
    ? record.citations.map((raw) => {
        ...
        if (
          ...
          || (page as number) < 1
          || (pageCount !== undefined && (page as number) > pageCount)
  ...
  let reconsideredPosition: ReaderReply["reconsideredPosition"];

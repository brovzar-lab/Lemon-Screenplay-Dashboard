...
export async function reserveLlmBudget(params: {
  ...
  await db.runTransaction(async (transaction) => {
    ...
    if (rolloutId !== undefined && rolloutId !== null) {
      ...
      const rolloutRef = db.collection(SYSTEM_COLLECTION).doc(`coverage-rollout-${rolloutId}`);
      const rolloutSnapshot = await transaction.get(rolloutRef);
      const rollout = rolloutSnapshot.data();
      if (rollout?.id !== rolloutId) throw new Error("Rollout document identity mismatch.");
      const jobs = rollout?.jobs;
      if (!jobs || typeof jobs !== "object" || Array.isArray(jobs)
        || Object.keys(jobs).length < 1 || Object.keys(jobs).length > 5) throw new Error("Missing bounded rollout binding.");
      const pilotSnapshots = await Promise.all(Object.keys(jobs).map((id) =>
        transaction.get(db.collection(INGEST_QUEUE_COLLECTION).doc(id))));
 ...
 */
export function admitRolloutExposure(
  raw: unknown,
  queues: Record<string, unknown>,
  jobId: string,
  requested: number,
): { admitted_exposure_microusd: number; admitted_attempts: number; jobs: Record<string, unknown> } {
  const object = (value: unknown): Record<string, unknown> => {
    if (!value || typeof value !== "object" || Array.isArray(value)) throw new Error("Malformed rollout state.");
    return value as Record<string, unknown>;
  };
  const integer = (value: unknown): number => {
    if (typeof value !== "number" || !Number.isSafeInteger(value) || value < 0) throw new Error("Malformed rollout counter.");
    return value;
  };
  ...
  if (rollout.status !== "active" || limit <= 0 || limit > 50_000_000
    ...
    || !allowedSources.every((value: unknown) => typeof value === "string" && /^[a-f0-9]{64}$/.test(value))
  ...
  for (const [id, value] of Object.entries(jobs)) {
    ...
    if (typeof binding.source_sha256 !== "string" || !/^[a-f0-9]{64}$/.test(binding.source_sha256)
      ...
      || job.rollout_release_sha !== rollout.release_sha) throw new Error("Rollout source or release mismatch.");
    ...
    if (sourceExposure > perSourceLimit || sourceAttempts > 3) throw new Error("Rollout source counters exceed limits.");
    ...
    if (integer(job.llm_active_reservation_count ?? 0) !== 0
      ...
      || integer(job.uncertain_cost_microusd ?? 0) !== 0) throw new Error("Rollout has unresolved accounting.");
  }
  ...
  if (exposure !== totalExposure || attempts !== totalAttempts) throw new Error("Rollout counters do not reconcile.");
  if (exposure + requested > limit || sourceExposure + requested > perSourceLimit
    || attempts >= 15 || sourceAttempts >= 3) throw new Error("ROLLOUT_LIMIT_REACHED: fixed exposure or attempt allowance exhausted.");
  return {

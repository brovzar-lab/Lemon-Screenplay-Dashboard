...
export const CALIBRATION_PROFILE_SCHEMA_VERSION =
  "lemon-calibration-profile-v1";
export const CALIBRATION_COMPILER_MODEL = "claude-opus-4-7";

const VERDICTS = ["pass", "consider", "recommend", "film_now"] as const;
const PURSUITS = ["no", "maybe", "yes"] as const;
const FIXABILITY = ["low", "medium", "high", "not_applicable"] as const;
...
export type DevelopmentFixability = typeof FIXABILITY[number];
export type ProducerConfidence = typeof CONFIDENCE[number];
export type TasteSignal = typeof TASTE_SIGNALS[number];
export type CalibrationPillar = typeof PILLARS[number];
...
export interface ProducerAnalysisSnapshot {
  ...
  pillarScores: Array<{ name: string; score: number; weight: number }>;
...
export function validateProducerJudgment(raw: unknown): ProducerJudgment {
  ...
  const tasteSignals = Array.isArray(judgment.tasteSignals)
    ? [...new Set(
      judgment.tasteSignals.map((item) =>
        enumValue(item, TASTE_SIGNALS, "Taste signal"),
      ),
    )].slice(0, TASTE_SIGNALS.length)
    : [];
  const rawOverrides = asRecord(judgment.pillarOverrides);
  const pillarOverrides: Partial<Record<CalibrationPillar, number>> = {};
  for (const pillar of PILLARS) {
    if (rawOverrides[pillar] !== undefined) {
      pillarOverrides[pillar] = finiteScore(
        rawOverrides[pillar],
        `${pillar} override`,
      );
    }
  ...
  return {
    ...
    producerVerdict: enumValue(
      judgment.producerVerdict,
      VERDICTS,
      "Producer verdict",
    ),
    pursuit: enumValue(judgment.pursuit, PURSUITS, "Pursuit decision"),
    fixability: enumValue(
      judgment.fixability,
      FIXABILITY,
      "Fixability",
    ),
...
function normalizeVerdict(value: unknown): RecommendationTier {
  const normalized = text(value).toLowerCase().replaceAll(" ", "_");
  return enumValue(normalized, VERDICTS, "AI verdict");
}

export function extractProducerAnalysisSnapshot(
  ...
  return {
    ...
    genre: text(
      asRecord(analysis.genre_classification).external_genre
        ?? analysis.genre
        ?? "Unspecified",
      200,
    ),
    ...
    aiRawScore: rawScore,
    aiVerdict: normalizeVerdict(
      analysis.verdict
        ?? asRecord(manifest.score_lineage).final_verdict,
    ),
    pillarScores,
    calibrationProfileVersionId:
      text(calibration.profile_version_id, 500) || null,
  };
}

export function buildProducerAssessment(input: {
  producerUid: string;
  producerEmail: string;
  producerDisplayName: string;
  judgment: ProducerJudgment;
  analysis: ProducerAnalysisSnapshot;
  prior?: ProducerAssessment;
  nowIso: string;
  assessmentId?: string;
}): ProducerAssessment {
  ...
  return {
    schemaVersion: PRODUCER_ASSESSMENT_SCHEMA_VERSION,
...
export function assessmentHeadId(
  producerUid: string,
  projectId: string,
): string {
  return `${documentId(producerUid, "Producer")}__${documentId(
    projectId,
    "Project",
  )}`;
}

export function buildAssessmentHead(assessment: ProducerAssessment) {
  return {
...
export function buildCompatibilityProjections(assessment: ProducerAssessment) {
  ...
  return {
    ...
    feedback: {
      ...
      dimensionOverrides: Object.fromEntries(
        analysis.pillarScores.map((pillar) => [
          pillar.name,
          {
            aiScore: pillar.score,
            userScore:
              judgment.pillarOverrides[pillar.name as CalibrationPillar]
              ?? pillar.score,
          },
        ]),
      ),
...
export function assessmentSetSha256(
  assessments: ProducerAssessment[],
): string {
...
function list(value: unknown, label: string, maxItems: number): string[] {
...
export function parseCalibrationPolicy(value: unknown): CalibrationPolicy {
  ...
  return {
    ...
    scoringInstructions: list(
      policy.scoring_instructions ?? policy.scoringInstructions,
      "Scoring instructions",
      12,
    ),
    developmentUpsideRules: list(
      policy.development_upside_rules ?? policy.developmentUpsideRules,
      "Development-upside rules",
      10,
    ),
    fixableWeaknessRules: list(
      policy.fixable_weakness_rules ?? policy.fixableWeaknessRules,
      "Fixable-weakness rules",
      10,
    ),
    dealbreakers: list(policy.dealbreakers, "Dealbreakers", 10),
    genreCautions: list(
      policy.genre_cautions ?? policy.genreCautions,
      "Genre cautions",
      10,
    ),
  };
...
export function renderCalibrationPrompt(policy: CalibrationPolicy): string {
...
export const CALIBRATION_POLICY_TOOL = {
...
export function buildDecisionReplayPrompt(
...
export const DECISION_REPLAY_TOOL = {
...
export function parseDecisionReplay(
  assessment: ProducerAssessment,
  value: unknown,
): DecisionReplay {
...
function isPositiveVerdict(verdict: RecommendationTier): boolean {
  return verdict === "recommend" || verdict === "film_now";
}

function isFalsePass(
  predicted: RecommendationTier,
  actual: RecommendationTier,
): boolean {
  return predicted === "pass" && isPositiveVerdict(actual);
}

function isFalseRecommendation(
  predicted: RecommendationTier,
  actual: RecommendationTier,
): boolean {
...
export function calculateCalibrationBenchmark(
  replays: DecisionReplay[],
): CalibrationBenchmark {
  if (!replays.length) {
    throw new Error("At least one holdout decision replay is required.");
  }
  const baselineErrors = replays.map((item) =>
    Math.abs(item.baselineScore - item.producerScore),
  );
  const candidateErrors = replays.map((item) =>
    Math.abs(item.candidateScore - item.producerScore),
  );
  const baselineAgreement = average(
    replays.map((item) =>
      item.baselineVerdict === item.producerVerdict ? 1 : 0,
    ),
  );
  const candidateAgreement = average(
    replays.map((item) =>
      item.candidateVerdict === item.producerVerdict ? 1 : 0,
    ),
  );
  const baselineFalsePasses = replays.filter((item) =>
    isFalsePass(item.baselineVerdict, item.producerVerdict),
  ).length;
  const candidateFalsePasses = replays.filter((item) =>
    isFalsePass(item.candidateVerdict, item.producerVerdict),
  ).length;
  const baselineFalseRecommendations = replays.filter((item) =>
    isFalseRecommendation(item.baselineVerdict, item.producerVerdict),
  ).length;
  const candidateFalseRecommendations = replays.filter((item) =>
    isFalseRecommendation(item.candidateVerdict, item.producerVerdict),
  ).length;
  ...
  return {
    ...
    passed:
      candidateFalsePasses <= baselineFalsePasses
...
export function confidenceForAssessmentCount(
  count: number,
): "early_signal" | "developing" | "reliable" {
  if (count >= 12) return "reliable";
  if (count >= 8) return "developing";
  return "early_signal";
}

export function buildCalibrationCandidate(input: {
  policy: CalibrationPolicy;
  training: ProducerAssessment[];
  ...
  return {
    ...
    profileId: "admin" as const,
    status: "candidate" as const,
    confidence: confidenceForAssessmentCount(
      input.training.length + input.benchmark.replays.length,
    ),
    compilerModelId: CALIBRATION_COMPILER_MODEL,
...
export function buildActiveCalibrationProfile(
  candidate: ReturnType<typeof buildCalibrationCandidate>,
  prior: UnknownRecord,
) {
  ...
  return {
    ...
    totalReviews:
      candidate.sourceAssessmentIds.length
